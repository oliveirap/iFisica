...
function circulosemborda(px,py,raio,cor1){
	ctx.beginPath();
	ctx.arc(px, py, raio, 0, 2*Math.PI, false);
	ctx.fillStyle = cor1;
	ctx.fill();	
...
function mola_01(x0,y0,N,passo,cor){
	var h = 6;
	var m = 1;
	var x = new Array();			
	//
...
//OBSERVADOR (ÓPTICA GEOMÉTRICA)
function olho(x1, y1,rotacao,e){
	var cor = 'rgb(0,0,120)';
	var dx = 25;
	var dy = 15;
	ctx.save();
	...
	ctx.stroke();
	//
	arco(x1,y1,25,-32,32,1,cor);
	arco(x1 + 36,y1,15,150,210,1,cor);
	//
	ctx.restore();
}
...
function lanterna(x1,y1){
	...
	ctx.beginPath();		
	ctx.moveTo(x1, y1);						
	ctx.lineTo(x1 + dx1, y1);
	ctx.lineTo(x1 + dx1 + dx2, y1 - dy1);
	ctx.lineTo(x1 + dx1 + dx2 + dx3, y1 - dy1);
	ctx.lineTo(x1 + dx1 + dx2 + dx3, y1 + dy2);
	ctx.lineTo(x1 + dx1 + dx2, y1 + dy2);
	ctx.lineTo(x1 + dx1, y1 + dy2 - dy1);
	ctx.lineTo(x1, y1 + dy2 - dy1);
	ctx.closePath();
	ctx.moveTo(x1 + dx1, y1);
	ctx.lineTo(x1 + dx1, y1 + dy2 - dy1);
